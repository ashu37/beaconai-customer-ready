...
**Addendum (S13.6-T1a, 2026-05-30):** S13.6-T1a ratified the strip — engine emits zero merchant-facing prose on contract surface. Stripped: `PlayCard.recommendation_text`, `PlayCard.why_now` (+ DECIDE copy ladder `_apply_copy_ladder`), `RejectedPlay.reason_text` / `evidence_snapshot` / `would_fire_if`, `Abstain.reason`, `notes: List[str]` debris on S6+ typed slots (gated `INCLUDE_DEBUG_FIELDS=False` default). `briefing.html` byte-identity pin retired in the same atomic commit; renderer stays runnable for dev convenience but is no longer load-bearing. Canary shifts to `engine_run.json` SHA + S13.7-T2 JSON-Schema round-trip. Source: founder + DS approval 2026-05-30 (`agent_outputs/code-refactor-engineer-s13.6-t1a-summary.md`; `agent_outputs/implementation-manager-s13.5-s13.6-s13.7-plan.md` REVISION HISTORY v2.1).

**S13.6-T6/T8 addendum (2026-06-01):** `PlayCard.mechanism_intent: Optional[MechanismIntent]` added to contract surface — typed enum atom, not prose. Renderer reads `mechanism_intent.type.value` directly; YAML-lookup fallback retired at T8. Engine emits zero merchant-facing prose on contract surface.
