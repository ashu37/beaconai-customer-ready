...
*Last updated: 2026-06-01 (post-S13.7 close).*
...
## 8. Approval-State Seam

Engine runs are immutable. Once `engine_run.json` is written, the engine does not modify it. Approval state (which PlayCards a merchant approves, rejects, or defers) lives in the agent database — not in the engine. The engine emits a typed slate; agents track what happens to it. This seam is intentional: it keeps the engine stateless and reproducible, and keeps approval state in a queryable store. Downstream: when a PlayCard is approved, the operator picks up `data/<store_id>/runs/<run_id>/audiences/<audience_definition_id>.csv` and uploads to Klaviyo. The engine does not send emails; it produces the audience list.

Source: `agent_outputs/ds-architect-end-to-end-flow-readiness.md` §founder action items #5; `docs/DECISIONS.md::D-S13.7-5`.
...
- `agent_outputs/ds-architect-end-to-end-flow-readiness.md` §founder action items #5 — approval-state seam doc origin; filesystem-only handoff posture
