"""S7.6-T3.5 atomic re-pin helper. Runs the same harness invocation the
Beauty pinned-slate test uses (see tests/test_slate_regression_beauty_brand.py
_B6_ENV_OVERRIDES) and copies briefing.html into the fixture path.

Also runs the supplements scenario and verifies its briefing is byte-
identical to the existing pin (helper short-circuits per Path-D Memo-4).
"""
...
def main() -> int:
    ...
    if s_old_sha == s_new_sha:
        ...
        print(f"Supplements DRIFT old={s_old_sha} new={s_new_sha} -- STOP")
        return 2
