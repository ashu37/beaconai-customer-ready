"""S7.6-T3.5 founder-tripwire probe (dry-run, no commits).

Runs the healthy_beauty_240d AND healthy_supplements_240d fixtures with
``ENGINE_V2_OBSERVED_EFFECT_DISCOUNT_HYGIENE=true`` and prints the
discount_dependency_hygiene card's blend_provenance so the IM/founder
can decide whether to flip the flag.

Supplements should short-circuit (Path-D Memo-4 REJECT) — no observed_*
fields populated.
"""
