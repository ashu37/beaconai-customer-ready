"""S7.6-T5.5 atomic re-pin helper. Mirrors scripts/s7_6_t6_5_repin.py.

Re-pins Beauty + Supplements pinned briefings after the
ENGINE_V2_OBSERVED_EFFECT_AOV_BUNDLE flag flip (OFF -> ON).

Expectation:
- Beauty re-pins: aov_lift_via_threshold_bundle Considered reason flips
  from cap_exceeded to signal_inconsistent_across_windows
  (joint-fail honest demotion).
- Supplements UNCHANGED: helper short-circuits per vertical_excluded_per_b5_248.
"""
