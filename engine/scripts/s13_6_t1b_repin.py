"""S13.6-T1b engine_run.json re-pin helper.

Founder + DS approved 2026-05-31 (atomic-split sibling of T1a per DS R5).
Computes post-strip engine_run.json SHA on the 5 pinned
synthetic_scenarios.yaml fixtures after stripping ``Observation.text``.
Modeled on ``scripts/s13_6_t1a_repin.py``.

Caveat (carried forward from S13-T3.5 and T1a): engine_run.json
contains wall-clock ``fit_timestamp`` values from S10-S12 ML
ModelCards, so the SHAs printed here record the at-commit moment only.
The load-bearing post-T1b test gates are the structural strip + AST
sweep + dataclass introspection tests in
``tests/test_s13_6_t1b_observation_text_strip.py`` and the extended
...
def main() -> int:
    print("S13.6-T1b engine_run.json SHA re-pin")
    print("=" * 60)
