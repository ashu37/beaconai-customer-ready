...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Note: AOV is a period-level metric (total_revenue/total_orders), not order-level.
    # Individual order amounts are available as 'net_sales'. Downstream code should
    # calculate AOV when needed from aggregated data.
...
def aligned_periods_summary(g: pd.DataFrame, min_window_n: int = 300) -> Dict[str,Any]:
    ...
    return {
        ...
        "recent_aov": float(recent["net_sales"].mean()) if len(recent)>0 else np.nan,
        "prior_aov": float(prior["net_sales"].mean()) if len(prior)>0 else np.nan,
