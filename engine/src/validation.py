"""
Enhanced Data Validation Engine for beacon
...
class AOVConsistencyCheck(ValidationCheck):
    ...
    def run(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if anchor is not None and 'Created at' in d.columns:
            ...
            d = d[(d['Created at'].dt.tz_localize(None) >= recent_start.tz_localize(None)) & (d['Created at'].dt.tz_localize(None) <= recent_end.tz_localize(None))]
...
class DataAnomalyCheck(ValidationCheck):
    ...
    def run(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'Created at' in df.columns:
            ...
            future_orders = df[df['Created at'].dt.tz_localize(None) > datetime.now()]
            ...
            very_old = df[df['Created at'].dt.tz_localize(None) < datetime.now() - timedelta(days=1095)]  # 3+ years
...
class AnomalousWindowCheck(ValidationCheck):
    """M1 T1.5: surface anomaly detector flags in the validation report.

    This check wraps :func:`src.anomaly.detect_anomalous_windows` and
    surfaces its result in the validation receipts. It does NOT gate any
    merchant-facing output (gating is M5). Status mapping:

    - no flags -> green
    - any single flag -> amber
    - ``insufficient_clean_history`` or 2+ flags -> amber (still not red)

    Per M1's hard constraint, the maximum severity here is ``amber`` so
    that downstream gating logic in ``main.py`` (which demotes on red)
    cannot accidentally change merchant output. M5 will introduce the
    proper ABSTAIN_HARD path.
    """

    def __init__(self):
        super().__init__(
            "Anomalous Window",
            "Detects BFCM overlap, refund storms, test-order anomalies, and insufficient history",
        )

    def run(self, data: Dict[str, Any]) -> Dict[str, Any]:
        # Lazy import: avoid circular imports at module load and keep this
        # check optional if someone strips the new modules in a hotfix.
        try:
            from .anomaly import detect_anomalous_windows
        except Exception as e:  # pragma: no cover - defensive
            return {
                "status": "green",
                "message": f"Anomaly detector unavailable: {e}",
                "details": {"flags": []},
                "severity": "info",
            }
        ...
        try:
            ...
            return {
                "status": "green",
                "message": f"Anomaly detector errored: {e}",
                "details": {"flags": []},
                "severity": "info",
            }
        ...
        if not flag_values:
            return {
                "status": "green",
                "message": "No anomalous-window signals detected",
                "details": {"flags": []},
                "severity": "info",
            }
        # M1: never escalate to red. M5 will add the gating layer.
        return {
            "status": "amber",
            "message": f"Detected anomaly flags: {', '.join(flag_values)}",
            "details": {"flags": flag_values},
            "severity": "warning",
        }
...
class DataValidationEngine:
    ...
    M1 T1.5 note: ``AnomalousWindowCheck`` is defined in this module but
    intentionally NOT registered in the default ``self.checks`` list at
    M1. Registering it would add a new key to ``validation_report.json``
    and break the M0 golden contract on fixtures whose analysis windows
    happen to overlap BFCM (e.g., ``small_sm`` with anchor 2024-12-31).
    The detector output is already surfaced in the new receipts file
    ``engine_run.json`` via the legacy adapter (T1.3). M5 will register
    this check here and gate on its output. To run the check explicitly
    in tests or downstream tooling, instantiate ``AnomalousWindowCheck``
    directly and call ``run({'df': df, 'aligned': aligned})``.
    """

    def __init__(self, *, include_anomaly_check: bool = False):
        # ``include_anomaly_check`` is opt-in; default False preserves M0
        # goldens. M5 will flip this default to True and gate accordingly.
        ...
        if include_anomaly_check:
            self.checks.append(AnomalousWindowCheck())
