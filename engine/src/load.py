...
def preprocess(df: pd.DataFrame):
    ...
    # Determine which revenue calculation to use based on data source
    if '_revenue_source' in df.columns and (df['_revenue_source'] == 'line_items_only').any():
        # Line items aggregation - net_sales already calculated correctly
        df["net_sales"] = df.get("line_items_net", df["Subtotal"] - df["Total Discount"])
        qa["revenue_calculation_method"] = "line_items_aggregation"
    else:
        # Standard order-level calculation
        net_from_sub = df["Subtotal"] - df["Total Discount"]
        net_from_tot = df["Total"] - df["Shipping"] - df["Taxes"]
        
        # Use consistent method across all rows - prefer subtotal method if available
        if net_from_sub.notna().sum() >= len(df) * 0.8:
            df["net_sales"] = net_from_sub
            qa["revenue_calculation_method"] = "subtotal_minus_discount"
        else:
            df["net_sales"] = net_from_tot
            qa["revenue_calculation_method"] = "total_minus_shipping_taxes"
    
    # Validate the calculation if we have both order and line item data
    if 'line_items_net' in df.columns and qa.get("revenue_calculation_method") != "line_items_aggregation":
        discrepancy = abs(df["net_sales"] - df["line_items_net"]) / df["net_sales"].replace(0, np.nan)
        qa["order_vs_line_items_discrepancy_mean"] = float(discrepancy.mean())
        qa["order_vs_line_items_discrepancy_max"] = float(discrepancy.max())
        if discrepancy.mean() > 0.1:  # 10% threshold
            qa["warning"] = f"Significant discrepancy between order and line-item calculations (avg: {discrepancy.mean():.1%})"
...
def load_csv(path: str, qa_out_path: str|None=None):
    # Use line-item aware loading for proper data handling
    df, has_line_items, items_df = load_orders_csv(path)
    _, qa = preprocess(df)  # Get QA metrics from preprocessing 
    qa['has_line_items'] = has_line_items
    qa['items_count'] = len(items_df) if items_df is not None else 0
...
def detect_line_item_format(df: pd.DataFrame) -> bool:
    ...
    signals = [
        ...
        (('quantity' in cols or 'lineitem quantity' in cols or 'lineitem_quantity' in cols) and 
         ('product' in cols or 'lineitem name' in cols or 'lineitem_name' in cols)),
    ]
...
def validate_price_calculations(items_df: pd.DataFrame) -> dict:
    """Validate price calculations for consistency and sanity checks"""
    issues = []
    
    # Check for unrealistic unit prices (> $1000)
    if 'unit_price' in items_df.columns:
        high_prices = (items_df['unit_price'] > 1000).sum()
        if high_prices > 0:
            issues.append(f"{high_prices} items with unit_price > $1000 - may indicate line totals instead of unit prices")
    
    # Check price-quantity consistency if we have both unit prices and line totals
    if all(c in items_df.columns for c in ['unit_price', 'quantity', '_line_total']):
        computed = items_df['unit_price'] * items_df['quantity']
        actual = items_df['_line_total']
        tolerance = 0.01  # 1 cent tolerance
        mismatches = (abs(computed - actual) > tolerance).sum()
        if mismatches > 0:
            issues.append(f"{mismatches} rows have price calculation mismatches (unit_price × quantity ≠ line_total)")
    
    # Check for zero or negative prices
    price_col = 'unit_price' if 'unit_price' in items_df.columns else '_line_total'
    if price_col in items_df.columns:
        zero_prices = (items_df[price_col] <= 0).sum()
        if zero_prices > 0:
            issues.append(f"{zero_prices} items have zero or negative prices")
...
def normalize_line_item_orders(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    items_df = items.copy()
    # Smart semantic-aware price handling to prevent double multiplication
    if 'quantity' in items_df.columns:
        items_df['quantity'] = pd.to_numeric(items_df['quantity'], errors='coerce').fillna(1)
    else:
        items_df['quantity'] = 1
        
    if 'line_item_discount' in items_df.columns:
        items_df['line_item_discount'] = pd.to_numeric(items_df['line_item_discount'], errors='coerce').fillna(0.0)
    else:
        items_df['line_item_discount'] = 0.0
    
    # Handle price semantics correctly based on what columns exist
    if 'unit_price' in items_df.columns:
        # CSV has unit prices - compute line totals correctly
        items_df['unit_price'] = pd.to_numeric(items_df['unit_price'], errors='coerce').fillna(0.0)
        items_df['_line_total'] = items_df['unit_price'] * items_df['quantity']
    elif 'line_item_total' in items_df.columns:
        # CSV has line totals - use directly, compute unit prices
        items_df['line_item_total'] = pd.to_numeric(items_df['line_item_total'], errors='coerce').fillna(0.0)
        items_df['_line_total'] = items_df['line_item_total']
        items_df['unit_price'] = items_df['line_item_total'] / items_df['quantity'].replace(0, 1)
    else:
        # Fallback: assume legacy 'line_item_price' field exists (backward compatibility)
    ...
    keep_first = {}
    for c in ['customer_email', 'created_at', 'customer_id']:
        if c in items_df.columns:
            keep_first[c] = 'first'
    # First aggregate line items with clear naming
    agg = items_df.groupby('order_id').agg({
        **keep_first,
        ...
        '_line_total': 'line_items_subtotal',
        '_line_discount': 'line_items_discount', 
        '_line_net': 'line_items_net',
        'quantity': 'line_items_quantity'
    })
    
    # Check if we have true order-level fields vs line-item-level fields
    order_level_cols = ['Subtotal', 'Total Discount', 'Shipping', 'Taxes', 'Total']
    has_potential_order_fields = any(col in df.columns for col in order_level_cols)
    
    # Determine if these are truly order-level by checking for consistency within orders
    truly_order_level = False
    if has_potential_order_fields:
        order_id_col = None
        for candidate in ['Name', 'Order ID', 'order_id']:
            if candidate in df.columns:
                order_id_col = candidate
                break
        
        if order_id_col and 'Subtotal' in df.columns:
            # Check if Subtotal is consistent within each order (truly order-level)
            # If it varies within an order, it's actually line-item level data
            # Handle NaN values properly: if all non-null values in an order are the same, it's order-level
            def check_order_level_consistency(group):
                non_null = group.dropna()
                return len(non_null.unique()) <= 1 if len(non_null) > 0 else True
            
            order_consistency = df.groupby(order_id_col)['Subtotal'].apply(check_order_level_consistency)
            truly_order_level = order_consistency.all()
    
    if truly_order_level:
        # These are genuine order-level fields - preserve them
        for col in order_level_cols:
            if col in df.columns:
                order_values = df.groupby(order_id_col)[col].first()
                order_values.index = order_values.index.astype(str).str.replace('#', '', regex=False)
                agg[col] = order_values
        agg['_revenue_source'] = 'true_order_level'
    else:
        # These are line-item-level fields disguised as order fields, or no order fields
        # Use our line item aggregations
        agg['Subtotal'] = agg['line_items_subtotal']
        agg['Total Discount'] = agg['line_items_discount']
        agg['Total'] = agg['line_items_net']  # Best approximation
        agg['Shipping'] = 0.0
        agg['Taxes'] = 0.0
        agg['_revenue_source'] = 'line_items_only'
    # Include appropriate price column in line_items_json based on what we have
    price_col = 'unit_price' if 'unit_price' in items_df.columns else 'line_item_price' if 'line_item_price' in items_df.columns else 'line_item_total'
    li_cols = [c for c in ['sku','quantity',price_col,'line_item_discount','variant_id','product_title'] if c in items_df.columns]
    ...
    orders_df['Subtotal'] = pd.to_numeric(orders_df.get('Subtotal', 0), errors='coerce').fillna(0.0)
    if 'Total Discount' in orders_df.columns:
        orders_df['Total Discount'] = pd.to_numeric(orders_df['Total Discount'], errors='coerce').fillna(0.0)
    else:
        orders_df['Total Discount'] = pd.to_numeric(orders_df.get('line_items_discount', 0), errors='coerce').fillna(0.0)
    ...
    orders_df['Lineitem quantity'] = pd.to_numeric(orders_df.get('line_items_quantity', 0), errors='coerce').fillna(0).astype(float)
    # Validate price calculations before returning
    validation_result = validate_price_calculations(items_df)
    if validation_result['issues']:
        print("⚠️  Price calculation warnings:")
...
def load_orders_csv(filepath: str, has_line_items: bool | None = None) -> Tuple[pd.DataFrame, bool, pd.DataFrame | None]:
    # Fail fast with a helpful message for empty/missing files
    try:
        p = Path(filepath)
        if not p.exists() or p.stat().st_size == 0:
            raise ValueError(
                f"Orders CSV is empty or missing: {filepath}. "
                "Provide a valid CSV with headers (e.g., 'beaconai/templates/samples/orders_sample.csv')."
            )
        raw = pd.read_csv(filepath, low_memory=False)
    except pd.errors.EmptyDataError:
        raise ValueError(
            f"Orders CSV appears empty: {filepath}. "
            "Provide a valid CSV with headers (e.g., 'beaconai/templates/samples/orders_sample.csv')."
        )
    ...
    if has_line_items:
        ...
        # Apply preprocessing to create required computed columns
        orders_df, _ = preprocess(orders_df)
