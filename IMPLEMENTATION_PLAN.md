...
- *Units.* A measurement is shown as the metric, its direction, and its window with the comparison — for example "Reactivation rate · Down 20.6% · last 56 days, compared with the 56 days before" — never as "Observed effect" or "L56". A sample figure appears only where the engine code establishes its unit: the lapsed cohort (reactivation), customers due to reorder (replenishment), first-time buyers (first-to-second), and identified customers (aligned-window metrics). "Orders analyzed" was wrong for every metric.
